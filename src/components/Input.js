...
export default class Input extends Component {
  ...
  _getTintColor() {
    const tintColor =
      this.state.status === undefined ? INPUT_TINT_COLOR : this._getColor();
    return tintColor;
  }

  _textInputColor() {
    return Platform.select({
      ios: { color: this._getColor() },
      android: undefined,
    });
  }
    ...
    return (
      <View>
        <TextField
          ...
          placeholderTextColor={this._getColor()}
          textInputStyle={this._textInputColor()}
          autoCorrect={false}
          onFocus={() => this.reset()}
